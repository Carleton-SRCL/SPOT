...
namespace std {
  ...
  inline ostream& operator<<(ostream &out, const OWL::Variant &v)
  { return out << "(" << (v.type_name()?v.type_name():"none") << ")" << v.str(); }
...
{ char *p=0; bool v=strtol(str.c_str(), &p, 10); return *p==0?v:def; }
...
template <> inline unsigned long get<unsigned long>(const std::string &str, const unsigned long &def)
{ char *p=0; unsigned long v=strtoul(str.c_str(), &p, 10); return *p==0?v:def; }
template <> inline long long get<long long>(const std::string &str, const long long &def)
{ char *p=0; long long v=strtoll(str.c_str(), &p, 10); return *p==0?v:def; }
template <> inline unsigned long long get<unsigned long long>(const std::string &str, const unsigned long long &def)
{ char *p=0; unsigned long long v=strtoull(str.c_str(), &p, 10); return *p==0?v:def; }
...
template <> inline bool get<uint8_t>(const std::string &str, uint8_t &v)
{ char *p=0; v=strtol(str.c_str(), &p, 16); return *p==0; }
...
{ char *p=0; v=strtol(str.c_str(), &p, 10); return *p==0; }
template <> inline bool get<unsigned long>(const std::string &str, unsigned long &v)
{ char *p=0; v=strtoul(str.c_str(), &p, 10); return *p==0; }
template <> inline bool get<long long>(const std::string &str, long long &v)
{ char *p=0; v=strtoll(str.c_str(), &p, 10); return *p==0; }
template <> inline bool get<unsigned long long>(const std::string &str, unsigned long long &v)
{ char *p=0; v=strtoull(str.c_str(), &p, 10); return *p==0; }
...
inline stringmap::stringmap(const std::string &str)
{
  ...
  for(strings::iterator i = l.begin(); i != l.end(); i++)
    if((s = split(*i, '=')).size() == 2) (*this)[s[0]] = s[1];
    else if(s.size() == 1) (*this)[s[0]] = "";
}
...
// return string after 'name' in 'src'
inline const char* find(const std::string &src, const std::string &name)
{
  size_t p = src.find(name);
  return p == std::string::npos ? 0 : src.c_str() + p + name.size();
}
...
{
  out << size << ": " << std::hex << std::setfill('0');
  for(size_t i = 0; i < size; i++) out << std::setw(2) << (int)(uint8_t)data[i] << " ";
...
namespace OWL {
  ...
  inline int64_t rescale(int64_t t, int64_t src, int64_t dst)
  {
    ...
    return (__int128(t) * __int128(dst) + src/2) / src;
  ...
  {
    if(src_num == dst_num && src_den == dst_den) return t;
#ifdef __SIZEOF_INT128__
    __int128 src = dst_num * src_den, dst = src_num * dst_den;
#else // !__SIZEOF_INT128__
    int64_t src = dst_num * src_den, dst = src_num * dst_den;
#endif // __SIZEOF_INT128__
    return (t * dst + src/2) / src;
  }

  inline bool operator!=(const Variant &a, const Variant &b)
  { return a.type_id() != b.type_id() || a.str() != b.str(); }
...
#define OWL_TYPEID(type, id) template <> inline TypeID<type>::operator uint32_t() const { return id; }
  ...
  OWL_TYPEID(PackInfo, Type::PACKINFO);
  ...
  inline void create(uint16_t id, void **begin, void **end, const void *first, const void *last)
  {
    switch(id)
      ...
      case Type::PACKINFO: return create_<PackInfo>(begin, end, first, last);
  ...
  inline void destroy(uint16_t id, void **begin, void **end)
  {
    switch(id)
      ...
      case Type::PACKINFO: return destroy_<PackInfo>(begin, end);
  ...
  public:
    ...
    uint16_t operator[](const std::string &name) const { return -1; }
  ...
  public:
    ...
    uint64_t operator[](const std::string &name) const { return -1; }

    void clear() { base::clear(); }
    T* set(uint64_t n) { return &base::operator[](n); }

    bool parse(const std::string &name, const std::string &options, const std::string &version) { return false; }
    bool parse_status(const std::string &name, const std::string &options, const std::string &version) { return false; }
  };
  ...
  public:
    ...
    inline Properties diff(const Properties &p) const; // this != p
    inline void merge(const Properties &p, bool full=true); // full ? this += p : p in this = p
  };
  ...
  public:
    ...
    virtual int read(int64_t timeout) = 0;
  ...
  public:
    ...
    virtual bool handle(EventPrivate &e, bool flag);
    virtual bool get(EventPrivate &frame);
    virtual int write(uint16_t type, const char *name, const char *data, size_t size);

    OWLAPI static Hybrid* create();
  };
  ...
  struct Frame {
    ...
    bool get(uint16_t id, EventPrivate &e);
    bool get(uint16_t id, OWL::Events &events);
  };
  ...
  public:
    ...
    Frames(uint16_t id=-1, int64_t capacity=0);
  ...
  public:
    ...
    FilterGroup(uint16_t id=-1, const std::string &name=std::string(), uint32_t period=0, bool enabled=false);
  ...
  inline Properties& Properties::operator=(const Properties &p)
  { std::map<std::string,Variant>::operator=(p); return *this; }
  ...
  Properties Properties::diff(const Properties &p) const // this != p
  {
    Properties r(types);
    for(const_iterator i = begin(); i != end(); i++)
      {
        const_iterator j = p.find(i->first);
        if(j == p.end() || i->second != j->second) r.insert(*i);
      }
    return r;
  }

  void Properties::merge(const Properties &p, bool full) // full ? this += p : p in this = p
  {
    if(full)
      ...
      for(iterator i = begin(); i != end(); i++)
        {
          const_iterator j = p.find(i->first);
          if(j != p.end()) i->second = j->second;
        }
  }

  //// TypeInfo ////

  template <> inline uint16_t table<TypeInfo>::operator[](const std::string &name) const
  { for(size_t i = 0; i < base::size(); i++) if(name == base::operator[](i).name) return uint16_t(i); return -1; }
        ...
        if(t && t->name)
          {
            ...
            if(v.size() > 1) t->flags = strtoul(v[1]);
            if(v.size() > 2) t->mode = strtoul(v[2]);
            continue;
          }

        t = set(n);
        *t = TypeInfo(v.size() > 1 ? strtoul(v[1]) : 0, v.size() > 2 ? strtoul(v[2]) : 0, strdup(v[0]));
      }
    return true;
  }
        ...
        if(o[0] == "id")
          {
            ...
            if(v.size() > 1)
              {
                d = 0;
                uint32_t id = strtoul(v[0]);
                if(id > 65535) continue;
                if(v.size() == 3) // id,tid,name
                  {
                    d = set(id);
                    d->id = id;
                    if(!v[1].empty()) d->tracker_id = strtoul(v[1]);
  ...
  // v2.2
  // status=devices hwid=hwid,time [status]
  // v2.1
  // status=devices id=hwid,time [status]
  template <> inline bool idmap<DeviceInfo>::parse_status(const std::string &name, const std::string &options, const std::string &version)
  {
    strings opts(split(options, ' '));
    if(opts.empty()) return false;

    bool api_2 = (version == "2.2");
    DeviceInfo *d = 0;
    if(opts[0] == std::string("status=") + name)
      {
        for(size_t i = 1; i < opts.size(); i++)
          {
            strings o = split(opts[i], '=');
            if(o.size() != 2) continue;
            if((api_2 && o[0] == "hwid") || (!api_2 && o[0] == "id"))
              {
                strings v = split(o[1], ',');
                if(v.size() > 1)
                  {
  ...
  // v2.2
  // table=devices hwid=n,hwid,type,name [options]
  // v2.1
  // table=devices id=hwid,id,type,name [options]
  template <> inline bool idmap<DeviceInfo>::parse(const std::string &name, const std::string &options, const std::string &version)
  {
    strings opts(split(options, ' '));
    if(opts.empty()) return false;
    ...
    if(opts[0] == std::string("table=") + name)
      {
        for(size_t i = 1; i < opts.size(); i++)
          {
            if(opts[i] == std::string("status=") + name) // special case
              return parse_status(name, options.substr(options.find("status="+name)), version);
                ...
                if(v.size() > 1)
                  {
                    d = 0;
                    uint64_t hwid = strtoull(v[0], 0, 0);
                    if(v.size() == 4) // n,hwid,type,name
                      {
                        d = set(hwid);
                        d->hw_id = strtoull(v[1], 0, 0);
                        if(!v[2].empty()) d->type = v[2];
                        if(!v[3].empty()) d->name = v[3];
                        d->options.clear();
                        if(d->hw_id == (uint64_t)-1 && erase(hwid)) d = 0;
                      }
                  }
              }
            else if(!api_2 && o[0] == "id")
              {
                strings v = split(o[1], ',');
                if(v.size() > 1)
                  {
  ...
  // table=pack type=type,id,name,id[,id,...] [options]
  inline bool pack_parse(std::vector<PackInfo> &packs, const std::string &name, const std::string &options)
  {
    strings opts(split(options, ' '));
    if(opts.empty() || opts[0] != std::string("table=") + name) return false;
    ...
    for(size_t i = 1; i < opts.size(); i++)
      {
        ...
        if(o[0] == "type")
          {
            strings v = split(o[1], ',');
            if(v.size() > 3) // type,id,name,ids
              {
                p = 0;
                uint32_t t = strtoul(v[0]);
                if(t >= Type::MARKERINFO) continue;
                packs.push_back(PackInfo(t));
                p = &packs.back();
                p->id = strtoul(v[1]);
                p->name = v[2];
                p->ids.clear();
                for(size_t j = 3; j < v.size(); j++) p->ids.push_back(strtoull(v[j], 0, 0));
              }
          }
        else if(p)
          {
            p->options += (p->options.empty() ? "":" ")+opts[i];
          }
      }
    return true;
  }
