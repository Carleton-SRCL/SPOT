...
public:
  ...
  int connect(const char *netaddress, unsigned short port, long long timeout);
  int connected(long long timeout);

  // flag: 1 - read
  // flag: 2 - write
  // flag: 3 - read+write
  int select(timeval &tv, int flag=0);
  int select(long long timeout, int flag=0);

  int peek() const;

  basic_socket accept(long long timeout);
...
public:
  ...
  int operator()(long long timeout, int flag);
