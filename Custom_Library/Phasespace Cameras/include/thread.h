...
public:
  ...
  bool wait(Mutex &mutex, long long timeout) // usec
  {
...
public:
  ...
  ReadLocker(ReadWriteLock *lock) : locked(false), lock(lock)
  {
    if(lock) lock->read_lock(), locked = true;
  }
  ...
  void relock()
  {
    if(lock && !locked) { lock->read_lock(); locked = true; }
  }

  void unlock()
  {
    if(lock && locked) { lock->unlock(); locked = false; }
  }
};
...
public:
  ...
  WriteLocker(ReadWriteLock *lock) : locked(false), lock(lock)
  {
    if(lock) lock->write_lock(), locked = true;
  }
  ...
  void relock()
  {
    if(lock && !locked) { lock->write_lock(); locked = true; }
  }
