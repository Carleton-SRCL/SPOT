...
const char OWL::VERSION[] = LIBOWL_VERSION;
#else
const char OWL::VERSION[] = "5.2";
#endif // LIBOWL_VERSION

//// utils ////

#ifdef WIN32
inline int64_t timer() // usec
{
...
int Context::open(const std::string &name, const std::string &open_options)
{
  ...
  int64_t timeout = get(m, "timeout", long(5000000)), t0 = timer(), left = 0;

  // keep internal events
  if(data->internal == 0) data->internal = get(m, "event.internal", int(0));
  ...
  data->initial = data->properties.diff(data->initial);
...
#endif // LIBOWL_VERSION
  if(!data->send("internal", out)) OPEN_ERROR(data->error, -1);
...
{ DATA_LOCKER(); return data ? data->isOpen() && data->property<int>("opened") == 1 : false; }
...
int Context::initialize(const std::string &init_options)
{
  ...
  int64_t timeout = get(m, "timeout", long(10000000)), t0 = timer(), left = 0;
  ...
  if(data->property<int>("initializing") == 0)
    {
      ...
      string o = string("event.raw=1 event.markers=1 event.rigids=1 event.info=1") + (init_options.empty()?"":" ") + init_options;
      data->enable(o);
      if(!data->send("initialize", o)) INITIALIZE_ERROR(data->error, -1);
    }
 ...
 finish:
  ...
  int hybrid = get(m, "hybrid", int(0));
  if(data->property<int>("local") == 1 && hybrid == 1 && data->hybrid == 0) data->hybrid = Hybrid::create();
...
int Context::done(const std::string &done_options)
{
  ...
  int64_t timeout = get(m, "timeout", long(1000000)), t0 = timer(), left = 0;
  vector<int> n;
  string keepalive = get(m, "keepalive", n) ? "keepalive=" + to_string(n[0]) : "";
  if(data->property<int>("initialized") == 0)
    {
  ...
  if(data->property<int>("flushing") == 0)
    {
      ...
      if(!data->send("done", keepalive)) DONE_ERROR(data->error, -1);
    }
...
bool Context::streaming(int enable)
{
  ...
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("streaming", vector<int>(1, enable));
}
...
bool Context::frequency(float freq)
{
  ...
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("frequency", vector<float>(1, freq));
}
  ...
  if(data->property<int>("local") == 1)
    {
      ...
      if(data->properties.get("missingsystemcameras", c))
        {
          EventPrivate e(Type::CAMERA, data->names["missingcameras"], 0, data->dev->time, c);
          data->new_event(e);
        }
    }
  // for local=0
...
bool Context::options(const std::string &options)
{
  ...
  CLEAR_ERROR();
  data->enable(options);
  return data->send("options", options);
}
...
bool Context::markerName(uint32_t marker_id, const std::string &marker_name)
{
  ...
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("markername", ostring() << "mid=" << marker_id << " name=" << marker_name);
}
...
bool Context::markerOptions(uint32_t marker_id, const string &marker_options)
{
  ...
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("markeroptions", ostring() << "mid=" << marker_id << " " << marker_options);
}

MarkerInfo Context::markerInfo(uint32_t marker_id) const
                            ...
                            const std::string &tracker_name, const std::string &tracker_options)
{
  ...
  return data->send("createtracker", out);
}
...
bool Context::destroyTracker(uint32_t tracker_id)
{
  ...
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("destroytracker", ostring() << "id=" << tracker_id);
}
...
bool Context::destroyTrackers(const uint32_t *first, const uint32_t *last)
{
  ...
  for(const uint32_t *i = first; i != last; i++)
    out << (i==first?"":",") << *i;
  return data->send("destroytracker", out);
}
                            ...
                            const std::string &marker_name, const std::string &marker_options)
{
  ...
  return data->send("assignmarker", out);
}
...
bool Context::trackerName(uint32_t tracker_id, const std::string &tracker_name)
{
  ...
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("trackername", ostring() << "id=" << tracker_id << " name=" << tracker_name);
}
...
bool Context::trackerOptions(uint32_t tracker_id, const std::string &tracker_options)
{
  ...
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("trackeroptions", ostring() << "id=" << tracker_id << " " << tracker_options);
}

std::vector<TrackerInfo> Context::findTrackerInfo(const std::string &type, const std::string &name) const
{
  DATA_LOCKER();
  std::vector<TrackerInfo> v;
  if(!data) return v;
  CLEAR_ERROR();
  for(table<TrackerInfo>::const_iterator i = data->trackers.begin(); i != data->trackers.end(); i++)
    if((type.empty() || type == i->type) && (name.empty() || name == i->name))
      v.push_back(*i);
  return v;
}

TrackerInfo Context::trackerInfo(uint32_t tracker_id) const
...
FilterInfo Context::filterInfo(const std::string &name) const
{ DATA_LOCKER(); return data ? data->filterInfo(name) : FilterInfo(); }

// devices //

bool Context::deviceOptions(uint64_t hw_id, const std::string &device_options)
{
  DATA_LOCKER();
  if(!data) return false;
  CLEAR_ERROR();
  return data->send("deviceoptions", ostring() << "hwid=0x" << hex << hw_id << dec << " " << device_options);
};

std::vector<DeviceInfo> Context::findDeviceInfo(const std::string &type, const std::string &name) const
{
  DATA_LOCKER();
  std::vector<DeviceInfo> v;
  if(!data) return v;
  CLEAR_ERROR();
  for(idmap<DeviceInfo>::const_iterator i = data->devices.begin(); i != data->devices.end(); i++)
    if((type.empty() || type == i->second.type) && (name.empty() || name == i->second.name))
      v.push_back(i->second);
  return v;
}

DeviceInfo Context::deviceInfo(uint64_t hw_id) const
{ DATA_LOCKER(); return data ? data->devices[hw_id] : DeviceInfo(); }

// pack ///

bool Context::pack(const PackInfo *first, const PackInfo *last)
{
  DATA_LOCKER();
  if(!data || !first || !last || first == last) return false;
  CLEAR_ERROR();
  ostring out;
  for(const PackInfo *i = first; i != last; i++)
    {
      out << (i==first?"":" ") << "type=" << i->type_id << " name=" << i->name;
      if(!i->ids.empty()) out << " ids=" << i->ids;
      if(!i->options.empty()) out << " " << i->options;
    }
  return data->send("pack", out);
}

std::vector<PackInfo> Context::packInfo() const
{ DATA_LOCKER(); return data ? data->packs : std::vector<PackInfo>(); }
...
Variant Context::property(const std::string &name) const
{
  DATA_LOCKER();
...
#ifdef LIBOWL_VERSION
const char OWL_VERSION[] = LIBOWL_VERSION;
#else
const char OWL_VERSION[] = "5.2";
#endif // LIBOWL_VERSION
...
{
  if(!ctx || !ctx->data()) { const OWLDeviceInfo r = { 0, 0, 0, 0, 0, 0}; return r; }
  const DeviceInfo &i = ctx->data()->devices[hw_id];
  const OWLDeviceInfo r = { i.hw_id, i.time, i.type.c_str(), i.name.c_str(), i.options.c_str(), i.status.c_str() };
  return r;
}

OWLAPI bool owlSetDeviceOptions(struct OWLContext *ctx, uint64_t hw_id, const char *device_options)
{ return ctx && device_options ? ctx->deviceOptions(hw_id, device_options) : false; }
