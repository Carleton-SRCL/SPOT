...
basic_socket::basic_socket(int domain, int type, int sock, const sockaddr &iaddr) :
  ...
  send_flags(MSG_NOSIGNAL), recv_flags(MSG_NOSIGNAL), oaddr(iaddr), iaddr(iaddr),
...
void basic_socket::nonblock(bool value) const
{
  ...
  if(ioctlsocket(_sock, FIONBIO, &argp))
    {
      std::cerr << "ioctlsocket: " << NET_ERROR_STR() << std::endl;
      exit(-1);
    }
#else // !WIN32
...
// return 0 on waiting
int basic_socket::connect(const char *netaddress, unsigned short port, long long timeout)
{
  if(socket() < 0) return _sock;
  ...
  int ret = ::connect(_sock, &oaddr, ADDRLEN);
  if(ret < 0 && !IS_NET_ERROR(EINPROGRESS, WSAEWOULDBLOCK))
    {
      derr << "error: connect to " << netaddress << ": " << NET_ERROR_STR() << endl;
      close();
      return ret;
  ...
  return connected(ret < 0 ? timeout : 0);
}
...
//  0 on waiting
// -1 on error
int basic_socket::connected(long long timeout)
{
  // wait for timeout
  int ret = select(timeout, 2+4);
  if(ret == 0)
    {
      // no timeout -- no error
      ...
      std::string e = NET_ERROR_STR();
      derr << "error: connect to " << getoaddr() << ": " << e << endl;
      return -1;
    }
...
int basic_socket::select(timeval &tv, int flag)
{
  ...
  if(flag & 4) FD_SET(_sock, &efds);
#else
  if(flag & 1) FD_SET(u_int(_sock), &rfds);
  if(flag & 2) FD_SET(u_int(_sock), &wfds);
  if(flag & 4) FD_SET(u_int(_sock), &efds);
#endif
  int ret = ::select(flag ? _sock+1 : 0, &rfds, &wfds, &efds, &tv);
...
int basic_socket::select(long long timeout, int flag)
{
  if(_sock < 0) return -1;
...
basic_socket basic_socket::accept(long long timeout)
{
  int new_sock = -1;
...
bool selector::read(int s) const { return FD_ISSET(s, &rfds) != 0; }
bool selector::write(int s) const { return FD_ISSET(s, &wfds) != 0; }
...
int selector::operator()(long long timeout, int flag)
{
  timeval tv = {timeout / 1000000, timeout % 1000000};
