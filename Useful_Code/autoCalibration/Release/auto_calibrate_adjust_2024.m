...
if dataType == 1
    ...
    % vectors representing the relative positions between pairs of markers (or LEDs) on a platform. These vectors are used to calculate the attitude (orientation) and position of the platform. Here's a breakdown of what each of these variables likely represents:
    % 
    % r_21: Vector from marker 2 to marker 1.
    % r_32: Vector from marker 3 to marker 2.
    % r_43: Vector from marker 4 to marker 3.
    % r_14: Vector from marker 1 to marker 4.
    % r_24: Vector from marker 2 to marker 4.
    % r_31: Vector from marker 3 to marker 1.
    % Each vector is defined by its x and y components in a two-dimensional space. For example, r_21 = [289;3] means the vector from marker 2 to marker 1 has an x-component of 289 and a y-component of 3.
    r_21 = [289;3];
    r_32 = [0;-248];
    r_43 = [-288;0];
    r_14 = [-1;245];
    r_24 = [288;248];
    r_31 = [289;-245];
    % r_21 = [1922.23-1643.12;403-404.864];
    % r_32 = [1643.12-1646.93;404.864-682.864];
    % r_43 = [1646.93-1925.68;682.864-679.986];
    % r_14 = [1925.68-1922.23;679.986-403];
    % r_24 = [1925.68-1643.12;679.986-404.864];
    % r_31 = [1922.23-1646.93;403-682.864];
    ...
    %filename = '2024-05-17 Calibration.csv';
    columns = 13; % how many columns are in the text file
    data = readmatrix(filename); % opening the file

    % originalFs = 955; % Original sampling frequency
    % targetFs = 20; % Target sampling frequency
    % downsampleFactor = originalFs / targetFs;

    
    t = data(:,1)';

    % [downsampledSignal, downsampledTime] = resample(data, t, targetFs);
    % 
    % t = downsampledTime;
...
data_end = input('Input where to end analysis (sample #): ');
close gcf


% Discarding bad data that may have been accumulated initially
theta = theta(data_start:data_end);
t = t(data_start:data_end);
t = t - t(1);
r_1x = r_1x(data_start:data_end);
r_1y = r_1y(data_start:data_end);
r_2x = r_2x(data_start:data_end);
r_2y = r_2y(data_start:data_end);
r_3x = r_3x(data_start:data_end);
r_3y = r_3y(data_start:data_end);
r_4x = r_4x(data_start:data_end);
r_4y = r_4y(data_start:data_end);
